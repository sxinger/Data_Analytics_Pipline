...
```
```{r}
grip_summ_r<-grip_summ %>% filter()
tbl3<-univar_analysis_mixed(id=grip_summ$Research_ID,
                            grp=grip_summ$summ_grip,
                            X=walkspeed_summ[,c("estimate","change_dir")],
                            data_type=c("num","cat"),
                            pretty=T,
                            digit=3)
...
```{r}
#
table(grip_summ$change_dir,
      grip_summ$Summary_Type)

prop.table(table(walkspeed_summ$change_dir,
           walkspeed_summ$Summary_Type),margin=2)

chisq.test(walkspeed_summ$Summary_Type,
           walkspeed_summ$change_dir)
...
```{r}
#
table(walkspeed_summ$change_dir,
           walkspeed_summ$Summary_Type)
